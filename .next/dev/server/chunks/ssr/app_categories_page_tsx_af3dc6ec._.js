...
function CategoriesPage() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        ...
        async function load() {
            setLoading(true);
            const { data: catData, error: catError } = await supabase.from("categories").select("id, slug, name, name_en, name_so").order("id", {
                ascending: true
            });
            const { data: subData, error: subError } = await supabase.from("subcategories").select("id, category_id, slug, name_en, name_so, img").order("id", {
                ascending: true
            });
            if (!catError && catData) setCats(catData);
            if (!subError && subData) setSubs(subData);
            setLoading(false);
        }
        load();
    }, []);
